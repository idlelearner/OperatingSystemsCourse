...
int scull_init_module(void){
  ...
  sema_init(&scullBufferDevice.sem_dev, 1);
  sema_init(&scullBufferDevice.sem_rw,1);
...
int scullBuffer_open(struct inode *inode, struct file *filp)
{
  ...
  if (down_interruptible(&dev->sem_rw))
    return -ERESTARTSYS;

  if (filp->f_mode & FMODE_READ){
    printk(KERN_DEBUG "increase reader\n");
    dev->readerCnt++;		//increase # of consumer
  }
  if (filp->f_mode & FMODE_WRITE){
    dev->writerCnt++;		//increase # of producer
    printk(KERN_DEBUG "increase writer\n");
  }	
  up(&dev->sem_rw);
  return 0;
}
...
int scullBuffer_release(struct inode *inode, struct file *filp)
{
  ...
  if (filp->f_mode & FMODE_READ){
    ...
    printk(KERN_DEBUG "decrease reader\n");
  }
  if (filp->f_mode & FMODE_WRITE){
    dev->writerCnt--;		//decrease # of producer
    printk(KERN_DEBUG "decrease writer\n");
  }	
  up(&dev->sem_rw);
  return 0;
}
...
start1:	if (down_interruptible(&dev->sem_dev))
          return -ERESTARTSYS;	
        ...
        if(dev->num_items<=0){
          ...
          if(dev->writerCnt>0){
            ...
            up(&dev->sem_rw);	
            up(&dev->sem_dev);	//go to the beginning
          ...
          printk(KERN_DEBUG "before read, num_items:%d,  next_item:%d position:%lld\n",dev->num_items,dev->next_item,*f_pos);
          next_item_size  = *(dev->itemSizes+dev->next_item);
          if(copy_to_user(buf,dev->bufferPtr+(loff_t)(dev->next_item*item_size), count)){
            countRead = -EFAULT;
            goto out;
          }
          dev->next_item = (dev->next_item+1) % scull_size;	//update the item pointer to the next possible item
          ...
          printk(KERN_DEBUG "after read,num_items:%d, next_item:%d\n\n",dev->num_items,dev->next_item);
        }
...
start2:	if (down_interruptible(&dev->sem_dev))
          return -ERESTARTSYS;	
        ...
        down(&dev->sem_empty);
        up(&dev->sem_full);
        printk(KERN_DEBUG "written %zd bytes  num_items=%d  slot=%d \n",count, dev->num_items,next_slot);
